...
# Make ordinal variables to a numeric for a more feasible analysis
  ...
  # Correction on Patient 27
    pre_post_dat2$GCS.zum.Zeitpunkt.der.Entlassung[27] = NA
    fu_dat2$mRS.30.Tage.postoperativ[27] = 6
  
  # Correction on Patient 16 CCI
    fu_dat2[16, 17] = 54.2
  ...
  # Complications
    # Patient 27 Complication Yes in FU
      #fu_dat2[27,10] = "Yes"
      # If no 27 not counted for FU, then:
      fu_dat2[27,11:15] = NA;
      # Post
...
summary_ord_groups_base = function(x, grp, decim = 1, suppress_NA = FALSE){
  ...
  if(is.factor(grp)){
   ...
   temp_df = data.frame(val = rank(x[!is.na(x)], na.last = NA), grp = grp[!is.na(x)])
   np_ana  = nparcomp::npar.t.test(val ~ grp, temp_df, nperm = 0, info = F)
...
summary_nom_groups = function(x, grp, suppress_NA = FALSE, decim = 1){
  ...
  #if(sum(!is.na(g1)) >= 4 & sum(!is.na(g2)) >= 4){
    pval_chisq = p_val_format(chisq.test(tot_tab_woNA)$p.value)
  #} else {
  #  pval_chisq = NA
  #}
...
cp_NNT_dec = round(1 / (cp_CER - cp_EER), 2)
```
...
The odds ratio of experiencing delirium (LA) to experiencing delirium (GA) together with it's asymptotic confidence interval in brackets is: `{r} or_wake_full` (`{r} ci[1]`, `{r} ci[2]`), i.e. the odds of having delirium after surgery with local anesthesia is `{r} or_wake_full` -fold. The Number Needed to Treat (NNT) is `{r} NNT` (`{r} NNT_dec`).
...
In total, between discharge and 30-day follow-up, `{r} complications_total_fu` (GA: `{r} compl_full_fu`; LA: `{r} compl_awake_fu`) patients experienced complications. Consequently, those who did not or those patients who died (Total: `{r} dead_total_fu`; GA: `{r} dead_full_fu`; LA: `{r} dead_awake_fu`) produce missing values for inapplicable variables (e.g.: Clavien-Dindo or GCS, respectively).
...
cp_NNT_fu_dec = round(1 / (cp_CER_fu - cp_EER_fu), 2)
```
...
The odds ratio of experiencing complications (LA) to experiencing complications (GA) at 30-day follow-up together with it's asymptotic confidence interval in brackets is: `{r} cp_or_wake_full_fu` (`{r} cp_ci_fu[1]`, `{r} cp_ci_fu[2]`), i.e. the odds of having complications after surgery with local anesthesia is `{r} cp_or_wake_full_fu` -fold. The Number Needed to Treat (NNT) is `{r} cp_NNT_fu` (`{r} cp_NNT_fu_dec`).
